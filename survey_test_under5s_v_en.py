...
# def survey():
#     save_path = '/home/vmoat/userstudy_wsbnew/answers/'
#     statistics = '/home/vmoat/userstudy_wsbnew/adv_statistics_11_level/adv_statistics_{}.csv'
#     statistics_files = [statistics.format(i) for i in range(9,-1,-1)] 
#     n_questions = [3, 3, 3, 3, 3, 3, 3, 3, 3, 3]
...
#     # for idx, csv_file in enumerate(statistics_files):
#     #     this_data = pd.read_csv(csv_file, index_col=0)\
#     #                         .sample(n=int(1), replace=False)      # 【s】改+1：  .sample(n=self.n_questions[idx]//2, replace=False)
#     #     pd_data_temp = pd.concat([pd_data_temp, this_data])   
#     # 前面的全是带adv的
#     for idx, csv_file in enumerate(statistics_files):
#         this_data = pd.read_csv(csv_file, index_col=0)\
#                             .sample(n=int(n_questions[idx]*1), replace=False)      # 【s】改：  .sample(n=self.n_questions[idx]//2, replace=False)
...
#         #                     .sample(n=int(n_questions[idx]*5/10), replace=False)      #  【s】改：  .sample(n=self.n_questions[idx]//2, replace=False)
...
#     this_data = pd.read_csv('/home/vmoat/userstudy_wsbnew/statistics/clean_statistics.csv', index_col=0)\
#                         .sample(n=int(15), replace=False)      #  【s】改：  .sample(n=self.n_questions[idx]//2, replace=False)
#     # random.shuffle(this_data)
#     pd_data = pd.concat([pd_data, this_data])
...
#     random_list = random_list + [0] * 15  #加上clean的
#     pd_data['option'] = random_list



#     # 全部打乱
#     pd_data = pd_data.sample(frac=1.0)    # 【s】改：是否进行全部打乱的选项，注释掉表示说不打乱了
#     # pd_data = pd.concat([pd_data_temp, pd_data])    #【s】pd_data表示前10个依次难度上升，然后后面的是pd_data是随机的


# # 问卷部分的 title
#     put_markdown("# User Study")
#     put_text("感谢您参加本次调查，请输入您的姓名，并回答接下来的问题：")
# # 必填
#     name = input("姓名")
#     info = session_info #表示会话信息的对象，属性 其中包括很多内容，可以自动获取
#     print(info)
#     user_ip = id(info['user_ip'])
#     clear()
    

# # 请听音频：
# # 本问卷旨在测试人耳是否具有分辨音频对抗样本的能力
# # 请选择安静环境，带上耳机，完成本问卷

# # 以下是试听环节：
# # 第一段音频为干净音频，第二段音频含有对抗噪声（5个level），请感受噪声的存在，并分辨两段音频

# # clear()

# # 以下是测试环节，
# # 请选出两段音频中含有对抗噪声的音频
#     for row in range(0,len(pd_data)):
#     # for row in range(0,2):
#         if pd_data.iloc[row,4] ==0:
#             wav_path1 = pd_data.iloc[row, 1]
#             wav_path2 = pd_data.iloc[row, 2]
#         elif  pd_data.iloc[row,4] ==1:
#             wav_path1 = pd_data.iloc[row, 2]
#             wav_path2 = pd_data.iloc[row, 1]
...
#         # noise_result = checkbox("请描述听到的两段音频", ["第一条有噪声", "第二条有噪声", "两段音频一样"])
#         put_markdown("> 请描述听到的两段音频")
#         tname = f"{pd_data.iloc[row, 0]}"
#         put_select(tname, options=[("第一条有噪声", 1), ("第二条有噪声", 0), ("两段音频一样", 2)])

#         # result.append(noise_result)
#         # clear()
#     # confirm = actions('作答完毕请提交问卷', ['提交'])   #作为一个延时
#     # input("是否提交")
#     put_markdown("# 问卷已完成，是否确认提交？")
#     button = put_button(['确认提交'], onclick=lambda: save_ans(pd_data, save_path, name))


# def save_ans(pd_data, save_path, name):
#     select_result = []
#     for row in range(0,len(pd_data)):
#     # for row in range(0,2):
#         tname = f"{pd_data.iloc[row, 0]}"
#         select_result.append(pin[tname])
#         # print(pin[tname])
#     # print(select_result)
#     # print(len(select_result))

#     pd_ans = collect_ans(pd_data, select_result)
#     qtime = time.strftime("%Y-%m-%d(%H:%M:%S)", time.localtime())
#     rand_name = random.randint(1,100000)

#     pd_ans.to_csv(f"{save_path}{name}{qtime}{rand_name:05d}.csv")
#     # 显示感谢信息
#     clear()
#     put_markdown("# 感谢您的参与！")
...
# def collect_ans(pd_data, select_result):
    
#     ans = pd_data
#     # print(len(ans))
#     # print(ans)
#     ans['select_result'] = select_result
#     return ans
